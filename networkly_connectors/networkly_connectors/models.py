...
- No `id`, `firm_id`, `first_seen`, `last_verified`, `last_checked`, or
  ...
  `opportunities` table (see `docs/build-plan.md` §2) that Networkly's ingest
...
- `region` exists as a field (the build-plan schema has one) but this
  ...
  *filter* (`_REGION_RE`, applied to decide whether to keep a row at all),
  ...
  judgment call Networkly's layer can make deliberately; guessing it here
...
class FetchResult:
    ...
    #: (see `networkly_connectors.zero_rows_guard`).
...
class Connector(Protocol):
    ...
    and it plugs into `networkly_connectors.fetch_many` /
    `networkly_connectors.verify` the same way Greenhouse/Lever/Workday do."""
