...
from networkly_connectors import lumesse
from networkly_connectors.models import LumesseBoard
...
def test_classify_url_recognizes_lumessetalentlink_apply_urls():
    ...
    nor verify(), so networkly_connectors.verify()'s dispatch loop raised a
...
def test_verify_is_reachable_through_the_public_dispatcher():
    """The end-to-end regression: networkly_connectors.verify() must not
    AttributeError on a Lumesse URL any more."""
    from networkly_connectors import verify as public_verify
    url = ("https://au01-apply.lumessetalentlink.com/apply-app/pages/"
           "application-form?jobId=X-1&langCode=en_GB")
    v = public_verify(url)
    assert v.provider == "lumesse"
