...
def test_all_tracked_jobs_load_and_ping_only_their_configured_url(
    ...
    loaded = runpy.run_path(find_spec("coverage_web.settings.base").origin)
...
def test_local_jobs_cannot_ping_production_and_leave_production_mapping_intact(
    ...
    from coverage_web.settings import base
    ...
    local = runpy.run_path(
        find_spec("coverage_web.settings.local").origin,
        run_name="coverage_web.settings._heartbeat_test",
    )
    ...
    production = runpy.run_path(
        find_spec("coverage_web.settings.production").origin,
        run_name="coverage_web.settings._heartbeat_test",
    )
