...
2. **No new-contact creation.** Mirrors `capture_gmail` (Step 1 of the daily
   ...
   touches against contacts ALREADY in Networkly; a message from someone not
...
3. **No push/webhook endpoint.** `users.watch()` needs a Pub/Sub topic, but
   ...
   whole feature works before Networkly has a public HTTPS deploy at all.
...
# "contacted before this contact was added to Networkly." A touched contact
...
# be a knob on how much of someone's mailbox Networkly reads.
#
...
def preview_sync(connection: GmailConnection) -> dict:
    ...
    "Writes nothing" means nothing in Networkly and nothing in the mailbox.
...
# `from:X OR to:X` for every contact already in Networkly, it can only ever
...
# Networkly feel alive does nothing on the one day it matters most. The
...
def backfill_connection(
    ...
    *,
    ...
    already in Networkly" may be nobody and the mailbox is the only thing
...
def backfill_new_contacts(user, contacts) -> "object | None":
    ...
    just because Networkly never checked Gmail — this runs the SAME
