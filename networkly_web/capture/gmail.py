"""Gmail-findings capture provider — the daily sync's route into Networkly.
...
layer pointed at Networkly instead, so one search feeds both systems and the
...
--------------------------------------------------------
`networkly_domain.pipeline` deliberately excludes the caller-side thread ratchet
...
from networkly_domain.pipeline import BULK_RECEIVED_KIND
...
class SyncResult:
    ...
    # Networkly failing to read it, which is why they are counted at all
    ...
    # The discovery hook (unmatched-sender branch only) raised on a
    # finding's own data and was caught rather than left to blow up the
    # batch — the third of the three per-finding hooks, on its own axis
    # next to `mail_facts_errors`/`app_events_errors` above so an operator
    # knows which layer to go fix. One malformed finding — an unexpected
    # shape from the search agent, a regex tripping over a header the
    # founder's real mailbox never sent before — must cost that finding's
    # proposal, never every OTHER finding still waiting in the batch.
...
class GmailFindingsProvider(CaptureProvider):
    ...
    Networkly resolves by email, then name, so the two systems' id spaces never
...
def _upsert_scheduled_chat(user, contact: Contact, finding: dict) -> bool:
    ...
    * NOTHING PROSE MAY OVERWRITE A STATED TIME. A row at
    ...
    # own comment draws the line: a `capture` row is something Networkly
...
def _retire_cancelled_chat(user, finding: dict) -> bool:
    ...
    * RETITLE ONLY, the way the .ics feed retitles a posting the firm pulled
    ...
    needs an argument, and the argument is symmetry. Networkly created this row
    ...
    state. The posture guards against Networkly INFERRING things into someone's
...
def _apply_findings(user, findings: list[dict], *, dry_run: bool, prepared) -> SyncResult:
    ...
    for index, finding in enumerate(findings):
        ...
        # contact (he wrote to them once, from outside Networkly), and the
        ...
        if contact is None:
            ...
            # not in Networkly wrote to you" surfaces. `consider_finding` runs
            ...
            # whose shape Networkly has seen the least of, so it is the most
        ...
        # human typed, in Networkly or in an earlier header, is left alone —
        ...
        # The note is the ONLY place a user ever sees why Networkly called a
...
def _stamp_subject(user, touch_result, finding: dict) -> None:
    ...
    `touches` INSERT lives in `networkly_domain.pipeline.apply_touch`'s raw SQL,
