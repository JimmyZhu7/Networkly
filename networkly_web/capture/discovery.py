...
Networkly". Two documents deliberately refused to build that:
...
4. Otherwise, a real human replying in a genuine thread (`In-Reply-To` /
   ...
   emailed them first, from outside Networkly.
...
student adds people to Networkly BEFORE emailing them. Measured on the
...
requests to bankers at directory firms — of which Networkly captured exactly
...
def _evidence_kind(finding: dict, *, outbound_only: bool = False) -> str:
    ...
    * `chat` sets warmth `chatted`, which `capture_worklist.RECHECK_WARMTH`
      ...
      put a person Networkly had never tracked into the one state no later run
...
def consider_finding(
    ...
    Networkly". Writes ONE row at most, and only ever a `ContactProposal`.
    ...
    # merge recipient who was never in Networkly (the outbound predates the
...
def school_firm_fields(contact, *, user=None, firm_domains: FirmDomains | None = None) -> dict:
    ...
    school — so they are off Firm Networkly, have no tier, no firm dates and no
...
def accept(
    ...
    *,
    ...
    if proposal.evidence_kind != "referral":
        ...
        # `networkly_domain`'s contract, and widening the pure engine for a
...
def restore(proposal: ContactProposal) -> tuple[str, Contact | None]:
    ...
    and in that gap the person may have entered Networkly by another door —
