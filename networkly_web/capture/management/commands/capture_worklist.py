...
run cannot leave a hole. That is right for someone Networkly has been watching
all along, and WRONG for someone who just arrived.

A contact with no touches has no history in Networkly at all. That is either
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help="Days for contacts Networkly already has history for "
    ...
    def handle(self, *args, **opts):
        ...
        self.stderr.write(
            f"{len(rows)} to re-check; {backfill} have no history in Networkly "
