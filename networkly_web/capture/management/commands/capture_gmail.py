...
at Networkly, and this command is its entry point — so one nightly search can
feed both systems.
...
def sync_window_days(user) -> int:
    ...
    Derived from the `Import` ledger rather than a state file — Networkly is
...
class Command(BaseCommand):
    ...
    def _report(self, result, *, dry_run: bool) -> None:
        ...
        self.stdout.write(
            ...
            f"{result.skipped_unmatched} unmatched in Networkly"
        )
