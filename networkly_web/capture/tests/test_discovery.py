...
from networkly_web.tenancy import TenantScopeError
...
def test_threaded_reply_from_personal_domain_proposes(student, firm):
    """The user emailed them first, from outside Networkly — still a
...
def test_a_proposal_never_carries_a_chat_rung(student, firm):
    ...
    * `chat` sets warmth `chatted`, which `capture_worklist.RECHECK_WARMTH`
      ...
      Networkly had never tracked lands in the one state no later run
...
def test_a_reply_to_a_declassified_campaign_is_never_proposed(student, firm):
    ...
    never in Networkly replies; their subject IS the campaign's signature. If
...
# directory firms — and Networkly captured exactly the two who replied within
