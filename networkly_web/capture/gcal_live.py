"""Google Calendar, mirrored one way: Google to Networkly, never back.

WHY THIS EXISTS. `CalendarEvent` already holds two kinds of row — a chat
Networkly read off an invite in the mailbox, and something the student typed
in — and its own docstring says the table renders "one timeline". The third
thing on a student's timeline is their actual calendar, and Networkly could
...
The student opens Networkly and sees the same chat twice, which is precisely
...
Google's attendee list is not where Networkly learned that.
...
* **No push.** Google Calendar has a watch/channel API that needs a public
  HTTPS endpoint, which means "deploy Networkly" becomes a precondition.
...
def is_configured() -> bool:
    ...
    reads the refusal as Networkly being broken.
    """
    return bool(
...
def _upsert_event(user, connection, event: dict, result: GcalSyncResult, *, dry_run: bool) -> None:
    ...
    An adopted row KEEPS ITS CONTACT and its kind. Networkly learned who the
