...
networkly_web/tenancy.py. Import/export use only the stdlib-compatible `csv`
...
def import_contacts(user, *, file_bytes: bytes, filename: str) -> ImportResult:
    ...
    # because Networkly never checked. Scoped to only the new rows (not the
...
_DELETE_ORDER: list[tuple[str, type]] = [
    ...
    ("imports", Import),
    ("contacts", Contact),
    # Talk to Networkly: messages reference `conversation` (CASCADE), and
