...
from accounts.adapter import NetworklyAccountAdapter, NetworklySocialAccountAdapter
...
def beta_settings(settings):
    ...
    settings.ACCOUNT_ADAPTER = "accounts.adapter.NetworklyAccountAdapter"
    settings.SOCIALACCOUNT_ADAPTER = "accounts.adapter.NetworklySocialAccountAdapter"
...
def save_google(login, form=None):
    ...
    with context.request_context(request):
        return NetworklySocialAccountAdapter(request).save_user(request, login, form=form)
...
def test_account_adapter_rejects_direct_local_save():
    request = auth_request()
    with pytest.raises(ImmediateHttpResponse):
        NetworklyAccountAdapter(request).save_user(request, User(email="bad@example.com"), SimpleNamespace(cleaned_data={}))
    assert not User.objects.exists()
...
def test_existing_social_login_is_allowed_without_invitation():
    ...
    login.user = user
    request = auth_request()
    NetworklySocialAccountAdapter(request).pre_social_login(request, login)
    assert not BetaInvitation.objects.exists()
...
def test_duplicate_email_is_blocked_before_allauth_fallback_or_mail(mailoutbox):
    ...
    with pytest.raises(ImmediateHttpResponse) as exc:
        NetworklySocialAccountAdapter(request).pre_social_login(request, google_login())
...
def test_beta_off_restores_local_signup(settings):
    ...
    assert NetworklyAccountAdapter(auth_request()).is_open_for_signup(auth_request())
