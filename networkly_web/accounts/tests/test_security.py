...
they rendered allauth's raw defaults — no Networkly shell, black text on the
...
# The card itself
# ---------------------------------------------------------------------------
def test_settings_renders_the_security_section(client, logged_in):
    body = client.get(reverse(SETTINGS)).content.decode()
    assert 'id="security"' in body
    assert 'aria-labelledby="security-h"' in body
...
# ---------------------------------------------------------------------------
# The allauth pages now render inside the Networkly shell
# ---------------------------------------------------------------------------
