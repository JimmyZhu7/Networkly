"""Talk to Networkly's own inline SVG icons must stay on the app-wide 1.5
...
that sweep missed (the JS-injected custom-select caret). Talk to Networkly
...
The one deliberate exception is the Networkly brand mark (`_logo.html`,
...
def test_every_talk_to_coverage_icon_ships_at_the_app_wide_stroke_width(signed_in, user):
    ...
    # scope for Talk to Networkly's own icon set.
    ...
    assert stroke_widths == {"1.5"}, (
        f"Talk to Networkly icon(s) drifted off the 1.5 stroke-width "
