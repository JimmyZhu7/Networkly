...
def test_search_contacts_last_touch_ignores_the_kinds_the_queue_ignores(user, firm):
    ...
    The definition is imported from `networkly_domain.cadence`, not restated
    here — this test pins that the tool agrees with the engine, and
    `networkly_domain/tests/test_stress_invariants.py` pins the set itself."""
    now = timezone.now()
...
def test_closing_within_days_says_it_returns_reported_deadlines_too(user):
    ...
    honest alternative to hiding a date Networkly read itself is labelling it,
...
def test_date_facts_reports_the_outreach_blackout_in_crm_todays_own_words(user):
    ...
    not about this feature (see `networkly_web/conftest.py`), and this one is.
