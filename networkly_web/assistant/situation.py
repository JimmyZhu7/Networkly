"""The situation snapshot: what changed in data Networkly already scrapes,
...
def _deadline_source(opp) -> str | None:
    ...
    Networkly's own regex read out of the posting's prose, "stated" for one
...
def _new_role_events(user, since, limit: int) -> list[dict]:
    ...
    # window just joined Networkly's board, and every role it has would read
    ...
    # brief — the first thing a new student ever reads from Networkly. See
