...
WHY THIS EXISTS: everything else on Talk to Networkly requires the student to
...
def _no_em_dashes(text: str) -> str:
    ...
    THE RULE: Networkly's copy has no em dashes. It is the founder's own,
...
def _summarize_actions(actions: list[dict], today: _date | None = None) -> str:
    ...
    for a in actions[:MAX_ACTIONS_SUMMARIZED]:
        ...
        # by networkly_domain.cadence from the linked firm first, falling
...
# marks a date as Networkly's own reading of a posting, and the words the
...
def _summarize_situation(events: list[dict], today: _date | None = None) -> str:
    ...
    for e in events[:MAX_SITUATION_SUMMARIZED]:
        ...
        if kind == "deadline_moved":
            ...
            # dates Networkly's regex read out of a posting's prose
...
def _is_stale(
    ...
    unconditionally, see `networkly_domain.pipeline.TOUCH_TRANSITIONS`; only
