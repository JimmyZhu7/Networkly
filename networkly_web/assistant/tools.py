...
`Model.objects.for_user(user)` (see `networkly_web/tenancy.py`: the default
...
from networkly_domain.pipeline import CHANNELS, THREAD_STATES, TOUCH_TRANSITIONS, WARMTH
...
# product's OWN quiet window, which is a fact about what Networkly will ask
...
def _log_touch(user, args, *, message_id: str) -> dict:
    ...
    `networkly_domain.pipeline`. Validated exactly as `crm.views.log_touch`
...
def _override_note(message_id: str, note: str, before: tuple[str, str]) -> str:
    ...
    THE PARENTHETICAL IS THE UNDO. `networkly_domain.pipeline.set_state`
