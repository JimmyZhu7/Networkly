...
package that does the bulk fetching, `networkly_connectors/http.py`, has
always identified itself honestly:

    networkly-connectors/0.1 (+https://coverage.app; deterministic ATS/board fetcher)
...
boundary: it decides whether Networkly sends a request at all. A disallowed
...
def _fetch_parser(robots_url: str, user_agent: str) -> RobotFileParser | None:
    ...
    which would make the one request Networkly sends about identity the one
