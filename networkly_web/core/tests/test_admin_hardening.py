...
def test_the_prefix_is_normalised_rather_than_trusted(monkeypatch, raw, expected):
    ...
    module = importlib.reload(importlib.import_module("networkly_web.settings.base"))
...
def moved_admin():
    ...
    `override_settings` alone is not enough: networkly_web/urls.py reads
    ...
    urls = importlib.import_module("networkly_web.urls")
...
def test_no_backfill_undo_file_is_tracked():
    """`networkly_web/region_backfill_undo_20260826T062308.json` was tracked
    ...
    listed = subprocess.run(
        ["git", "ls-files", "--", "networkly_web/*_undo_*.json"],
