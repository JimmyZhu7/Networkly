"""The Django -> networkly_domain adapter (docs/build-plan.md §2, "the
domain adapter").

Thin wiring only — no business logic lives here. `networkly_domain.pipeline`
...
def log_touch(
    ...
    *,
    ...
    Thin wrapper over `networkly_domain.pipeline.apply_touch` — see that
...
def set_contact_state(
    ...
    *,
    ...
    — see `networkly_domain.pipeline.set_state`.
