...
`networkly_web.tenancy.PrivateModel` — see that module for the
tenant-scoped-manager contract.

Schema-compatibility note (critical — see the task brief and
networkly_domain/networkly_domain/pipeline.py's module docstring): `Contact`
...
from networkly_web.tenancy import PrivateModel
...
class Contact(PrivateModel):
    ...
    # entirely by networkly_domain.pipeline's ratchet, not Django.
...
class Touch(PrivateModel):
    ...
    `networkly_domain.pipeline.apply_touch` / `set_state`, via
    ...
    # INSERTed by `networkly_domain.pipeline.apply_touch`'s raw SQL, whose column
...
class Campaign(PrivateModel):
    ...
    Networkly ingested every one of them as his personal recruiting network, and
    ...
    # 41 people the detector had grouped out of Networkly's own boilerplate
...
class CalendarEvent(PrivateModel):
    ...
    WHY THIS TABLE EXISTS. Networkly knew a chat had been *scheduled*
    ...
    # `capture` row is something Networkly FOUND in the student's mail and can
    # be argued with; a `gcal` row is the student's own calendar restated,
    # read-only, and the place to change it is Google Calendar. Networkly
