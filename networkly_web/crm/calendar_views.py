...
def _events_by_day(user, first: date, last: date) -> dict[date, list[dict]]:
    ...
    for ev in (CalendarEvent.objects.for_user(user)
        ...
        buckets.setdefault(day, []).append({
            ...
            # because that is where Networkly first learned about it and the
...
def calendar_delete(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    a second opinion about it. The event lives in Google; Networkly holds a
...
def _own_manual_event(user, pk: int) -> CalendarEvent:
    ...
    * `source="capture"` is a chat Networkly found in the mailbox, and
