...
from networkly_domain import cadence
from networkly_domain.pipeline import (
    BULK_RECEIVED_KIND,
    REFERRAL_KIND,
    TOUCH_TRANSITIONS,
)
...
# has to stay inside. Everything else in networkly_domain.cadence.CADENCE_DEFAULTS
...
# Networkly's own sends. The current support is one practitioner source at
...
def _build_actions(user, *, pace: bool = True):
    ...
    # `networkly_domain` is pure and cannot reach `CalendarEvent`, so a chat's
    ...
    #
    # `networkly_domain` cannot reach `ChatDebrief`, so "this person said they
    ...
    actions = _gate_and_rank(actions, tiers, openings, sent_today, today,
                             ...
                             # Derived from the board Networkly watches, never
...
def _opening_keep_warms(
    ...
    WHY THE VIEW RAISES THESE AND THE ENGINE CANNOT. `networkly_domain.cadence`
...
#   - replied-parked: already gets `networkly_domain.cadence` branch 3's
...
# outcomes: nobody has yet observed a Networkly user's reply rate against firms
...
def _mark_undeliverable(user, actions: list[dict]) -> None:
    ...
    Networkly still asks me to follow up." Measured read-only on his account
    ...
    WHY HERE AND NOT IN THE ENGINE (E4). `networkly_domain` is pure: no Django,
...
def _gate_and_rank(actions: list[dict], tiers: dict, openings: dict,
    ...
    module docstring for why none of it belongs in `networkly_domain.cadence`.
...
def _today_sort_key(a: dict):
    ...
    `networkly_domain.cadence.due_actions`' own sort was given a contact-id
...
# `networkly_domain.cadence.business_days_since` counts Mon-Fri with no holiday
...
def outreach_blackout(today) -> str | None:
    ...
    default, `networkly_web/conftest.py`) pins an ordinary weekday by patching
...
def _recent_activity(user, *, as_of) -> list[dict]:
    ...
    def _rail_ago(ts) -> str:
        ...
        # `networkly_domain.cadence` and `.scoring` both guard this same
...
# `networkly_domain`, which is pure and has no business knowing what time a
...
def _next_deadlines(user, today, limit=4) -> list[dict]:
    ...
    for fd in rows:
        ...
        out.append({
            ...
            # opening Networkly actually watched. None renders as nothing at
...
def _gap_role(opportunity) -> dict:
    ...
    Networkly's own reading of a posting's prose, and a bare date on this
...
def plan_split(actions: list[dict], *, cap: int, today) -> dict:
    ...
    #
    # WHY HERE AND NOT IN `networkly_domain.cadence`. The layering rule is
...
def _reschedule_chat(user, contact, starts_at, *, now):
    ...
    `networkly_domain.pipeline.TOUCH_TRANSITIONS` defines it — the objection
...
def today_act(request: HttpRequest, pk: int, verb: str) -> HttpResponse:
    ...
    `replied` (`networkly_domain.pipeline.TOUCH_TRANSITIONS`) on the strength
...
def _prose_dates(reason: str, *, today=None) -> str:
    ...
    the engine's raw fragment stays untouched at the source (networkly_domain
...
def _age_in_days(reason: str, hours: float | None, *, now) -> str:
    ...
    (networkly_domain is another workstream) — the action dict already carries
...
def _sentenceize(reason: str) -> str:
    ...
    engine's raw fragments stay untouched at the source (networkly_domain is
...
def _dashboard_context(user) -> dict:
    ...
    # Networkly read the date out of the posting's prose
