...
WHY THIS IS NOT IN THE ENGINE. `networkly_domain.cadence`'s own docstring draws
...
1. RELEVANCE (`contact_relevance`). Measured on the founder's own 156-contact
   ...
   Tacori, WME, a law firm — asking them to speak on a club panel. Networkly
...
#
# WHAT WOULD CHANGE IT: a counted reply rate by rung on real Networkly sends.
...
# `SYNTHESIS-PLAN.md` Part C item 6). So the season is read off Networkly's own
...
# Networkly will be able to compute one from its own sends; it cannot yet.
...
#
# WHAT WOULD RETIRE THE RULE: Networkly measuring its own reply rate by mode.
...
def season_mode(user, today=None) -> str | None:
    ...
    Networkly watches for this student, what share have already been observed
    ...
    for (_firm_id, _region), obs in rows.items():
        ...
        # evidence of a quiet market, it is evidence of a board Networkly has
...
# Move it forward."
#
# WHY THE REWRITE IS HERE AND NOT IN THE ENGINE. `networkly_domain.cadence`
