...
Companion to `networkly_domain/tests/test_stress_invariants.py`, which does the
...
def test_a_covered_track_seat_is_never_hidden(role):
    ...
    assert verdict.verdict == recruitment.KEEP, (
        f"{role!r} is a seat on a track Networkly covers and was hidden as "
...
def test_every_app_authored_template_is_refused_a_signature(note):
    ...
    column, all fell through to a note Networkly had written itself, and the
    identical string read as a blast. Every template Networkly composes must be
...
def test_boilerplate_wrapped_around_a_real_send_still_groups():
    ...
    that was Networkly's own words WRAPPED AROUND something the sender wrote."""
...
# guarantee `networkly_domain.cadence.due_actions`' own sort was given a
