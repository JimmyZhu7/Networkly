...
openness, which counts up from an open Networkly watched and predicts nothing.
...
def test_onboarding_batch_postings_do_not_unlock_the_line(client):
    """A firm whose entire visible board arrived on the day Networkly started
...
# and `networkly_domain.cadence._closing_soon` buckets re-pings by it — the
