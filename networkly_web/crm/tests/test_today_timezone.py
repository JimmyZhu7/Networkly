...
def test_the_confirm_chat_card_never_states_a_scheduling_date_it_lacks():
    ...
    She has no `CalendarEvent` — the ordinary state, since Networkly only
...
def test_a_real_calendar_event_lets_the_card_name_the_day():
    """The other half: when Networkly DOES hold a time, the card says it, in
...
# `networkly_domain.cadence` and `.scoring` both treat a future-dated touch as
