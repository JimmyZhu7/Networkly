...
The Networkly Gaps strip that used to sit above these cards, and which kept
...
def _tier_board(body: str) -> str:
    """Just the tier cards, not the board around them. The Networkly Gaps
...
def test_no_surface_on_this_page_wears_an_open_count_any_more(client, student):
    ...
    card in the Networkly Gaps strip, which was a different component from
