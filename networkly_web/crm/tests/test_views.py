...
def test_a_bulk_blast_alone_lands_a_cold_contact_in_not_contacted_not_emailed(client):
    ...
    note. `networkly_domain.cadence` already excludes `manual_override` and
...
def test_the_staleness_ring_ignores_a_manual_override_after_the_real_last_touch(client):
    ...
    touch. `networkly_domain.cadence`'s own idle-clock math already excludes
...
def test_max_cold_touches_cannot_be_raised_past_two():
    ...
    networkly_domain.cadence trusts whatever `params` it's handed by design;
