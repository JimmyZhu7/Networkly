"""Track fit, recruiting style and the advocate aggregate in the Networkly
...
def test_network_page_ranks_with_tracks_and_style_and_exposes_the_aggregate(client):
    ...
    # were bolted on by the view for the deleted Networkly Gaps row's own
...
def test_the_advocate_line_is_nowhere_on_the_page(client):
    ...
    Networkly Gaps heading's `title=`. That heading is gone (2026-09-02), so
    ...
    assert "Networkly Gaps</h2>" not in body, "the heading is back"
