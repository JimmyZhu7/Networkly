...
#
# `networkly_domain.pipeline.CHANNELS` is the PORTED tuple and is not this: the
...
# what the original CLI offered, and this list is what Networkly offers.
...
# every automatic path Networkly has. A channel value is the cheapest possible
...
def _mailto(to_email: str, *, subject: str = "", body: str = "") -> str:
    ...
    composes start from Networkly so a contact's opener is one click away.
...
def _touch_dicts(touches) -> list[dict[str, Any]]:
    ...
    Harmless for the other consumer. `networkly_domain.scoring` measures in
...
# key and `networkly_domain.cadence._closing_soon` buckets by it. Three
...
def firm_date_market(region) -> str:
    ...
    a market Networkly has not mapped yet is still a market the row states,
...
# (`crm.today._next_deadlines`), the Network board's own Networkly-Gaps
...
def firm_date_confidence(fd) -> str:
    ...
    (`networkly_domain.cadence`, `scoring.score_firm`), which only ever act on
    "confirmed_official".
