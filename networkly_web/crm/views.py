...
§6 fit score). Mounted at /app/ (see networkly_web/urls.py); every view is
login-required and scopes every private-zone read with
`.for_user(request.user)` (see networkly_web/tenancy.py).
...
from networkly_domain import cadence, scoring
from networkly_domain.pipeline import MANUAL_OVERRIDE_KIND, TOUCH_TRANSITIONS
...
# was the "Who to find" panel on a Networkly Gaps row, and that strip is gone
...
def _pick_lever(contacts):
    ...
    Shared by the Networkly Gaps strip and every firm card on the board
...
# `networkly_domain.cadence`'s own `_CLOCK_SILENT_KINDS` — same private-name
...
def _stale_window_days(c, params) -> int:
    from networkly_domain.cadence import CADENCE_DEFAULTS
...
def contact_list(request: HttpRequest) -> HttpResponse:
    ...
    School), Firm Networkly grouped by the user's own tiers (draggable —
    ...
    # Networkly, the warmth sections and the contact count is not an answer to
    # the question Settings asked him.
    #
    # Removed BEFORE the scope filter and before every count below, so the
    # tier board, the Networkly Gaps strip, the action lanes, `contact_total`
    ...
    # total, the tier board, Networkly Gaps, the action lanes, the warmth
    ...
    # Networkly holds, and it can say it once with all four numbers instead of
    ...
    # --- Firm Networkly, grouped by the user's tiers ---------------------
    user_firms = list(
    ...
    # down because the firm cards need it too now — the Networkly Gaps strip
    ...
    def firm_card(uf):
        ...
        return {
            ...
            # feed's whole job and the Networkly Gaps strip above still names
            ...
            # used, and the same red mono countdown the Networkly Gaps strip
            ...
            # What the Networkly Gaps strip already computes for its worst 6
            ...
            # The "CG" tag: everything the deleted Networkly Gaps strip said,
...
def sourcing_event(request: HttpRequest) -> HttpResponse:
    ...
    NO CALLER IN THE APP as of 2026-09-02. The panel lived on a Networkly
...
#     `networkly_domain.cadence`'s branch order. Unaffected by this feature.
...
def _contact_live_context(
    ...
    if firm is not None:
        ...
        # Labelled here rather than in networkly_domain: the enum is what the
