...
bonus, when it has something real to say) what's new for you — Networkly's
...
that Networkly already knows something happened in the student's world this
...
def _coverage_summary(user, *, today: date) -> dict:
    ...
    from exactly what the Network board's Networkly Gaps strip and the
...
def _new_for_you(user, *, today: date) -> tuple[list[dict], str, str]:
    ...
    Networkly's own reading, and an email cannot underline."""
