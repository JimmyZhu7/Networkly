...
applying" — and Networkly already reads that mailbox twice a day. So the board
fills itself, and the student's job shrinks to correcting it.

FINDINGS SHAPE
--------------
A JSON array of `{"firm", "title", "applied_at", "evidence", "thread_id"}`.
`firm` is a Networkly slug or a firm name; `title` is the role as the
...
def _resolve_firm(raw: str):
    """A Networkly slug, or a firm name, or nothing."""
    name = (raw or "").strip()
    if not name:
        return None
