...
- Going where it is not wanted, or pretending to be someone else. Every
  request names Networkly (see `UA` below) and is checked against the host's
...
# `networkly_connectors/http.py`'s, and every user-agent in this project now
...
# and `networkly_connectors.workday._normalize()` stores that count verbatim
...
# query (networkly_connectors/goldmansachs.py) never carries — confirmed live
...
# networkly_connectors/oracle.py already calls for board fetch/verify answers
...
# as a `sitemap` board — see networkly_connectors/sitemap.py) states its
...
# A sitemap board (see networkly_connectors/sitemap.py) has no title field of
