...
that nothing in Networkly reads — so a scan could confirm a real deadline and
Networkly would never learn it. This command is that missing half.
...
A JSON array. Each entry:
    ...
    {
      "firm": "goldman-sachs",          # Networkly firm slug (required)
...
1. CONFIDENCE NEVER SILENTLY DOWNGRADES. A date already recorded as
   ...
   posture (see networkly_domain.cadence) reads off exactly that field.
