...
URLs, so `networkly_connectors.workday.verify()` 422'd on every one of these
...
`networkly_connectors.workday.verify()` — ids 9530 ("Service Transition &
...
- Opportunity id=9595 (BMO, "Personal Banking Associate", source="phenom",
  ...
  tenant the row's stored URL actually resolves into. `networkly_connectors.
...
- Opportunity id=3979 (Millennium, "Data Operations Analyst - Systematic
  ...
  paging or truncation bug in `networkly_connectors/eightfold.py`, whose
...
from networkly_connectors import verify
