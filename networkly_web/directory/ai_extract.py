...
says nothing — Networkly's whole pitch is that a chip's evidence is checkable.
...
class AIExtractError(Exception):
    ...
    last underlying exception, mirroring networkly_connectors.http.FetchError's
...
def _post_json(payload: dict, *, timeout: float, retries: int) -> dict:
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            # networkly_connectors.http's unretried 404.
            if e.code < 500:
