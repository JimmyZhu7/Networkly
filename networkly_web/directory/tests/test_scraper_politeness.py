...
Networkly, and no outbound request happens against a path the host has
...
def _wall(origin: str) -> None:
    ...
    fixture (networkly_web/conftest.py) keeps the fetch path off the network
...
def test_both_follow_the_connectors_user_agent_convention(ua):
    """`networkly_connectors/http.py`'s shape: a bare product token, then a
