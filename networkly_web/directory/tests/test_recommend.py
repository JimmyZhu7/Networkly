...
def test_a_programme_year_is_inference_and_is_labelled_as_such():
    ...
    say the posting did not state this and that Networkly inferred it. What
...
def test_a_stated_market_outside_the_profile_costs_more_than_an_unread_one():
    ...
    A blank region is Networkly failing to read a location and is charged
    against Networkly (`W_REGION_UNKNOWN`, -8). A region we DID read and that
...
# its date — the firm still lists it, and Networkly does not close what the
