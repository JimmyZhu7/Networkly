...
def school_region(school: str) -> str:
    ...
    three answers this function must not pass on:
      ...
      "other"   a stated place outside the six markets Networkly tracks. It is
...
_ROLE_FUNCTION: tuple[tuple[re.Pattern[str], str], ...] = tuple(
    (re.compile(rx, re.I), track) for rx, track in (
        ...
        # Banking and Advisory (Networkly)" at SocGen), so it named no
...
#: Functions that are NOT any of the tracks Networkly lets a student pick.
...
_NON_TRACK_FUNCTION = re.compile(
    ...
    # and the corporate/commercial/transaction banking arm. Networkly lets a
...
def _network_fit(profile: Profile, c: Candidate) -> tuple[int, list[Reason]]:
    ...
    real contact at SIG should be able to see that Networkly read the
...
def recommend(
    ...
    *,
    ...
    for c in candidates:
        ...
        # and Networkly does not close what the source has not — but a PICK is
