...
`scrape` command feeds to `networkly_connectors.fetch_many`.
...
constant. Networkly's `firms` schema is owned by another workstream and must not
...
from networkly_connectors import (
...
BOARDS: list[tuple[str, BoardConfig]] = [
    ...
    # gate; Networkly will not bypass detection, so Bain stays cycle-dates-
    ...
    # `successfactors` connector (see networkly_connectors/successfactors.py).
    ...
    # invisible to Networkly and the catalog could not have known, because the
...
# ---- Networkly gaps against the founder's target universe (probed 2026-07-25) ----
#
...
#    disallowed. Networkly does not fetch a site a tenant asks it not to —
...
# Campus boards Networkly knows the address of and will NOT fetch, because the
...
# inventory this product exists to surface. Networkly still does not fetch
...
# the campus programme on a site Networkly does not query. Slug -> the board
