...
in it. Networkly's earliest `first_seen` anywhere is 2026-07-23. In a 39-day
...
that rule's own reason. On the first day Networkly ever scraped a firm's
...
def onboarding_cutoffs(firm_ids=None, *, fresh: bool = False) -> dict[int, date]:
    ...
    Networkly started watching a firm, it moves once per firm ever, and the
...
def open_run_days(opp, today: date, cutoffs: dict[int, date]) -> int | None:
    ...
    * `first_seen` falls on or before the firm's onboarding day, where it
      records when Networkly arrived, not when the posting did (see the module
