...
- Testability/greppability: `pytest networkly_web/directory` can assert on
...
def deadline_marker(deadline, precision, *, today=None):
    ...
    exactly the job "how long has Networkly known about this" is useful for,
...
# past its stated date is still genuinely accepting applications. Networkly
...
def _unconfirmed_note(o, *, as_of_date=None) -> dict:
    """Whether Networkly's own most recent check of this posting actually
...
def _urgency_item(o, *, now, today, my_firm_ids, profile=None, cutoffs=None):
    ...
    item = {
        ...
        # `open_run_days`, because their `first_seen` records when Networkly
        ...
        #
        # Distinct from `seen_days`, which is how long Networkly has KNOWN
    ...
    # passed" for the same row — two Networkly surfaces disagreeing about the
...
def opportunities(request, *, pick_only=False):
    ...
    #      the posting (Networkly's own first_seen — a signal no ATS exposes).
    ...
    context = {
        ...
        # told Networkly (target firms, survey regions/tracks). Computed above
...
def firm_detail(request, slug):
    ...
    context = {
        ...
        # firm HAS a campus board, Networkly knows its address, and Networkly
...
def _people_at_firms(user, firm_ids, *, today, cap) -> dict:
    ...
    (networkly_web/tenancy.py) — `Contact.objects` unscoped raises
