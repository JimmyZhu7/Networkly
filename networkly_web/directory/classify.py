...
never invent taxonomy (see networkly_connectors/models.py), so every posting
...
REGION_LABELS = {
    ...
    # A real market Networkly does not track (Sydney, Mumbai, Seoul, São
...
# The markets Networkly actually tracks, in display order. This is the set a
...
# Markets Networkly does not track, recognised so they can be FILED rather than
...
# Commodities, Global Networkly Centre, Global Investment Research, SIG's
...
# market Networkly does not track, so the honest answer is `other`, not blank.
...
def normalize_region(location: str | None, *, fallback: str = "") -> str:
    ...
    # market Networkly does not track files under "other" instead of blank.
...
def bucket_from_contract(contract_type: str | None, title: str = "") -> str:
    ...
    "Graduate Trainee in CIB Networkly 企业金融部管培生-北京" — a
...
def posting_text(title: str | None, raw: dict | None) -> str:
    ...
    Networkly's own keys are skipped — see `_OURS_NOT_THE_POSTINGS`."""
