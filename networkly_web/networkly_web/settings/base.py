...
`manage.py` / `wsgi.py` / `asgi.py` default to `networkly_web.settings.local`
...
# This file lives at networkly_web/networkly_web/settings/base.py.
# parents[0] = settings/, [1] = networkly_web/ (the Django package dir, also
# BASE_DIR — where manage.py, core/, templates/, static/ all live).
BASE_DIR = Path(__file__).resolve().parents[2]

# Repo root, one level above the networkly_web package — where a local .env
...
# Where the Django admin is mounted (networkly_web/urls.py reads this).
#
...
INSTALLED_APPS = [
    ...
    "assistant",  # private zone: "Talk to Networkly" — the advisor page's conversations
...
# only the weekly digest email's "open in Networkly" links (crm/digest.py,
...
# ---------------------------------------------------------------------------
# "Talk to Networkly" — the advisor page (assistant/).
#
...
# surfaces that spend real model money: "Talk to Networkly" chat messages
...
#     spec at all. Networkly has 156 such attributes across 22 templates, 43
...
CONTENT_SECURITY_POLICY = {
    "DIRECTIVES": {
        ...
        # in browsers that support this directive — Networkly is never meant
...
# above). Networkly uses none of these browser features anywhere — no camera/
...
ROOT_URLCONF = "networkly_web.urls"
...
WSGI_APPLICATION = "networkly_web.wsgi.application"
ASGI_APPLICATION = "networkly_web.asgi.application"
...
ACCOUNT_ADAPTER = "accounts.adapter.NetworklyAccountAdapter"
...
# Networkly is broken (`audit-security.md` finding 17). A placeholder is not a
...
# means "deploy Networkly" becomes a precondition. Pull means
...
# unreadable, which reads to a student as "Networkly silently disconnected my
...
#
# `calendar.readonly`, never `calendar` or `calendar.events`. Networkly
...
# Networkly being broken. Flip this in the environment on the same deploy
...
SOCIALACCOUNT_ADAPTER = "accounts.adapter.NetworklySocialAccountAdapter"
